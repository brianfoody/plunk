...
export default function PaginatedTable({
	...
	return (
		<div className="space-y-4">
			...
			{(totalPages > 1 || selectable) && (
				<div className="flex justify-between items-center">
					<span className="text-sm text-gray-600">
						{totalPages > 1 && `Page ${page} of ${totalPages} - ${total} total items`}
						{selectable && (isSelectingAll ? allSelectedCount > 0 : selectedIds.length > 0) && (
							<>
								{totalPages > 1 && " "}({isSelectingAll ? `${allSelectedCount} selected` : `${selectedIds.length} selected`}
								)
							</>
						)}
					</span>
